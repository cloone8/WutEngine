...
use syn::{punctuated::Punctuated, Ident, Index, LitInt, Token};
...
fn make_componentfilter_impl(configs: Vec<ComponentConfig>) -> TokenStream {
    ...
    quote! {
        impl<'a, #types> ComponentFilter<'a> for (#impl_types)
        where
            #wheres,
        {
...
pub fn make_componentfilter_tuples_impl(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    let max_length: LitInt = syn::parse(input).unwrap();

    let mut output = TokenStream::new();

    for i in 2..=max_length.base10_parse().unwrap() {
        output.extend(make_componentfilter_tuple(i));
    }
