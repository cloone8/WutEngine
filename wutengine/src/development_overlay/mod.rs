...
use crate::graphics::mesh::IndexBuffer;
use crate::graphics::mesh::VertexBuffer;
use crate::graphics::sampler::Sampler;
use crate::graphics::shader::GVec2;
...
pub(crate) fn render_if_active(
    ...
    if let Some((vertex_buffers, index_buffer)) = gather_primitive_buffers(&clipped_output) {
        ...
        render_pass.set_index_buffer(index_buffer.buffer.slice(..), index_buffer.format.to_wgpu());
    ...
    match primitive.primitive {
        egui::epaint::Primitive::Mesh(egui_mesh) => {
            ...
            pass.set_bind_group(
                ...
                tex_mat.material.user_bind_group.get_bind_group().unwrap(),
                &[],
            );
            ...
            for (attr_type, attr_info) in attrs {
                ...
                pass.set_vertex_buffer(
                    ...
                    vertex_buffer.buffer.slice(start_bytes..end_bytes),
                );
            }
    ...
    for (tex_id, delta) in to_set {
        ...
        match delta.pos {
            ...
            None => {
                ...
                let mut material =
                    Material::new(crate::builtins::shaders::EGUI.get_arc().unwrap(), map![]);
                ...
                material.user_bind_group.update_bind_group(device);
