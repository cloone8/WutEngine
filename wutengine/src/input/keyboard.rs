...
const MAX_KEYCODE: usize = 2usize.pow((size_of::<KeyCode>() * 8usize) as u32) - 1;
...
impl KeyboardInputPlugin {
    ...
    pub fn new() -> Self {
        Self {
            key_states: [false; MAX_KEYCODE],
        }
    }
}

impl WutEnginePlugin for KeyboardInputPlugin {
    fn on_device_event(&mut self, device: DeviceId, event: &DeviceEvent, _commands: &mut Command) {
        match event {
...
const fn winit_keycode_to_usize(key: KeyCode) -> usize {
    match key {
        KeyCode::Backquote => (0),
        KeyCode::Backslash => (1),
